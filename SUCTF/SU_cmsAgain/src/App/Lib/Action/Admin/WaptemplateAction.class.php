 ...
 */
class WaptemplateAction extends AdminBaseAction {
	...
	function index(){
		...
		for($i = 0; $i < $count; $i++){
			...
			if( $FileList[$i]['isDir'] == 1 ){
				...
				$FileList[$i]['fileurl'] = ($CurrentDir == '/') ?  $t['WapTheme'].$FileList[$i]['filename'] :  $t['WapTheme'].$CurrentDir.'/'.$FileList[$i]['filename'];
		...
		$this->assign('TemplateName', $t['WapName']);
	...
	function modify(){
		...
		$FullFileName = TMPL_PATH.'Wap/'.$ThemeName.'/'.ltrim($FileName,'/');
	...
	function saveModify(){
		...
		$FullFileName = TMPL_PATH.'Wap/'.$ThemeName.'/'.ltrim($_POST['FileName'],'/');
		if( !$this->isValidTplFile($FullFileName, 'Wap')){
	...
	function pick(){
		...
		for($i = 0; $i < $count; $i++){
			if( $FileList[$i]['isDir'] == 1 ){
				$name = $h['pWapTpl'].$FileList[$i]['filename'].'/template.xml';
				if( file_exists($name) ){
					$xml = @simplexml_load_file( $name );
					$t[$i]['name'] = (string)$xml->name;
					$t[$i]['dirname'] = $FileList[$i]['filename'];
					$t[$i]['thumbnail'] = $h['WapTpl'].$FileList[$i]['filename'].'/'.$xml->thumbnail;
					$t[$i]['IsUse'] = ( strtolower($t[$i]['dirname']) == strtolower($h['WapThemeName'] ) ) ? 1 : 0;					
	...
	function style(){
		...
		import("ORG.Io.Dir");
		$d = new Dir( $t['pWapCss'].$tplCurrentDir );
		$FileList = $d->toArray();
		$count = count($FileList);
		for($i = 0; $i < $count; $i++){
			$FileList[$i]['mtime'] = yd_friend_date($FileList[$i]['mtime']);
			if( $FileList[$i]['isDir'] == 1 ){
				...
				$FileList[$i]['size'] = byte_format( $FileList[$i]['size'] );
				$FileList[$i]['filetype'] = getTplFileType( $FileList[$i]['filename'] );
				$FileList[$i]['fileurl'] = ($tplCurrentDir == '/') ?  $t['WapCss'].$FileList[$i]['filename'] :  $t['WapCss'].$tplCurrentDir.'/'.$FileList[$i]['filename'];
				$extFile = './Public/Images/FileICO/'.$FileList[$i]['ext'].'.gif';
				if( is_file( $extFile ) ){
					$FileList[$i]['ico'] = $this->WebPublic.'Images/FileICO/'.$FileList[$i]['ext'].'.gif';
				}else{
	...
	function modifyStyle(){
		...
		$FileName = YdInput::checkFileName($_GET['file']);
		$ThemeName = C('WAP_DEFAULT_THEME');
		$FullFileName = TMPL_PATH.'Wap/'.$ThemeName.'/Public/css/'.ltrim($FileName,'/');
	...
	function backup(){
		...
		if(is_dir( $tdir )){
			...
			$zipfile = APP_DATA_PATH.'zip/wap_'.$tname.'_'.date("Y-m-d_H_i_s", time()).'_'.rand_string(6,10).'.zip';
	...
	function lang(){
		$tname = YdInput::checkFileName(trim( $_GET['tname'] ));  //当前模板目录
		if( empty($tname) ){
			$tname = C('WAP_DEFAULT_THEME');
		}
		$dir = TMPL_PATH.'Wap/'.$tname.'/Lang/';
	...
	private function _saveLangFile($data, $dirName='Default', $languageMark='cn'){
		$dir = TMPL_PATH.'Wap/'.$dirName.'/Lang/';
		if( !is_dir($dir) ){
			mk_dir($dir);
		}
