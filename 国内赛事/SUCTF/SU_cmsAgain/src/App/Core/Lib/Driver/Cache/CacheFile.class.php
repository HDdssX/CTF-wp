...
class CacheFile extends Cache {
    ...
    private function filename($name) {
        //$name	=	md5($name);
        //使用DATA_CACHE_KEY混淆缓存文件名，防止getshell攻击
        //$name = md5(C('DATA_CACHE_KEY') . $name);
        //3.2.4版增加了DATA_CACHE_KEY参数，默认为_think_，一下使用User和Pwd作为cachekey
        $cachekey = C('DB_USER').C('DB_PWD');
        $name = md5($cachekey . $name);
    ...
    public function set($name,$value,$expire=null) {
        ...
        //详见文章：ThinkPHP3.2.3~5.0.10缓存函数设计缺陷可导致Getshell
        //https://blog.csdn.net/Fly_hps/article/details/84952890
        //$data = str_replace(PHP_EOL, '', $data); //2019-05-30
