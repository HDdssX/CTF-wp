...
class TagLibCx extends TagLib {
    ...
    protected $tags   =  array(
        ...
        'php'=>array(),
        'volist'=>array('attr'=>'name,id,offset,length,key,mod','level'=>5),
    ...
    public function _volist($attr,$content) {
        ...
        if( empty($tag['offset']) ) $tag['offset']=0; //by wang 解决只设置length会报错，offset默认为0
        // 允许使用函数设定数据集 <volist name=":fun('arg')" id="vo">{$vo.name}</volist>
        $parseStr   =  '<?php ';
        
        //让$empty支持变量功能，by wang=======================
        if('$' == substr($empty, 0, 1)) {
        	$empty  =  $this->autoBuildVar(substr($empty,1));  //多语言变量返回: L("NoInforation")
        	$parseStr   .= '$_empty='.$empty.';';  //防止缓存
        	$parseStr   .= "\$_empty=str_replace(array('[',']','^^','^'), array('<','>','\"',\"'\"), \$_empty);";
        	$empty = '$_empty';
        	/*
        	 if( 'L("' == substr($empty,0,3) ){ //语言变量
        	 		$len = strlen($empty) - 5;
        	 		$lname = substr($empty, 3, $len);
        	 		$empty = L("$lname");
        	 		}
        	 		*/
        }else{
        	$empty = str_replace(array('[',']'), array('<','>'), $empty);
        }
        //===========================================
