...
class Model {
    ...
    /**
     *  实现group_concat(CouponSendID)功能
     * @param $field
     * @param int $type 1返回字符串，2数组
     */
    public function groupField($field, $type=1) {
        $options['field'] = $field;
        $options =  $this->_parseOptions($options);
        $resultSet = $this->db->select($options);
        $data = array();
    ...
    protected function _validationFieldItem($data,$val) {
        switch($val[4]) {
            ...
            case 'unique': // 验证某个值是否唯一
                ...
                return true;
                case 'lang_unique': //by wang 2012-12-05 验证某个值是否唯一, 支持多语言
                	if(is_string($val[0]) && strpos($val[0],','))
                		$val[0]  =  explode(',',$val[0]);
    ...
    function getAttachment($id){
    	$flag = $GLOBALS['Config']['AUTO_DEL_ENABLE'];
    	if( $flag == 0 || $id=='') return false;
    	$modelName = $this->getModelName(); //获取模型名称
    	$id = YdInput::filterCommaNum($id);
    	$idList = is_array($id) ? implode(',', $id) : $id;
    	$fileList = array();
    	switch( strtolower($modelName) ){
    		case 'info':
    			$this->field('InfoPicture,InfoAttachment,InfoContent');
    			$data = $this->where("InfoID in($idList)")->select();
    			foreach($data as $v){
    				if( !empty($v['InfoPicture']) ) {
    				    $fileList[] = $v['InfoPicture'];
                    }
    				if( !empty($v['InfoAttachment']) ) {
    				    $fileList[] = $v['InfoAttachment'];
                    }
    				if( !empty($v['InfoContent']) ){
    					$t = yd_extract_image($v['InfoContent'], 0);
    					foreach($t as $it){
    						$fileList[] = $it;
    					}
    				}
    			...
    			break;
    		case 'banner':
    			$data = $this->field('BannerImage')->where("BannerID in($idList) and BannerImage!='' ")->select();
    			foreach($data as $v) {
    			    $fileList[] = $v['BannerImage'];
                }
    			break;
    		case 'ad':
    			//1:图片广告，2：Flash广告，5：对联广告
    			$data = $this->field('AdContent,AdTypeID')->where("AdID in($idList) and AdTypeID=1 and AdContent!=''")->select();
    			foreach($data as $v){
    				$fileList[] = $v['AdContent'];
    			}
    			break;
    		case 'link':
    			$data = $this->field('LinkLogo')->where("LinkID in($idList) and LinkLogo!='' ")->select();
    			foreach($data as $v){
    			    $fileList[] = $v['LinkLogo'];
                }
    			break;
    		case 'channel':
    			$data = $this->field('ChannelPicture,ChannelContent')->where("ChannelID in($idList) ")->select();
    			foreach($data as $v) {
    				if( !empty($v['ChannelPicture']) ) {
    				    $fileList[] = $v['ChannelPicture'];
                    }
    				if( !empty($v['ChannelContent']) ){
    					$t = yd_extract_image($v['ChannelContent'], 0);
    					foreach($t as $it){
    						$fileList[] = $it;
    					}
    				}
        ...
        foreach($fileList as $file){ //只能删除Upload目录下的文件
            if('/Upload/' == substr($file, 0, 8)){
     ...
     */
    function baseDel($id, $options=array()){
    	//获取模型名称
    	$ModelName = $this->name;
    	if( is_array($id) ){
    		$n = count($id);
    		...
    		$where = "{$ModelName}ID in(".implode(',', $id).')';
    	}else{
     ...
     * @param array $options
     */
    function baseSort($id=array(), $order = array(), $options=array()){
    	$n = is_array($id) ? count($id) : 0;
    	$ModelName = $this->name;
    	for($i = 0; $i < $n; $i++){
    		if( is_numeric($order[$i]) && is_numeric( $id[$i] ) ){
    			$this->where("{$ModelName}ID=".$id[$i])->setField("{$ModelName}Order", $order[$i]);
    		}
    	}
    }
     ...
     * @param array $options
     */
    function baseDelAll($options=array()){
    	$result = $this->where('1=1')->delete();
    	return $result;
    }
}